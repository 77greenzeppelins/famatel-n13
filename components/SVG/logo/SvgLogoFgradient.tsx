/**BasicData*/
import { corpoColors } from '../../../data/_data';
/**FramerMotion Staff*/
import { motion } from 'framer-motion';
// import { svgTransition } from '../../../utils/framerMotion/framerMotionUtils';
// const textVariants = {
//   strokeLight: { fill: corpoColors.light, transition: { duration: 0.4 } },
//   strokeDark: { fill: corpoColors.dark, transition: { duration: 0.4 } },
// };

/**------------------------------------------------------------------------------*/
const SvgLogoFgradient = ({
  animationCondition = false,
  // basicSize
  svgStyle,
  gStyle,
}: {
  animationCondition?: boolean;
  svgStyle: string;
  gStyle?: string;
  //  basicSize: number
}) => {
  /**...WTF*/
  // console.log('SvgLogo', animationCondition);
  /**FramerMotion Staff*/

  /**JSX*/
  return (
    <motion.svg
      role="img"
      aria-hidden="true"
      xmlns="http://www.w3.org/2000/svg"
      className={svgStyle}
      // width={basicSize}
      // height="500"
      viewBox="0 0 358 500"
      // animate={animationCondition ? 'strokeDark' : 'strokeLight'}
      // whileHover={{ scale: 1.05 }}
      // transition={svgTransition}
    >
      <defs>
        <linearGradient id="linearGradient3330">
          <stop offset="0" stopOpacity="0" stop-color="white"></stop>
          <stop offset="0.25" stop-color="white"></stop>
          <stop offset="0.75" stopOpacity="0.839" stop-color="white"></stop>
          <stop offset="1" stopOpacity="0" stop-color="white"></stop>
        </linearGradient>
        <linearGradient
          id="linearGradient3284"
          x1="185"
          x2="346.2"
          y1="383.21"
          y2="383.21"
          gradientTransform="matrix(1 0 0 .1 0 145.32)"
          gradientUnits="userSpaceOnUse"
          xlinkHref="#linearGradient3330"
        ></linearGradient>
        <linearGradient
          id="linearGradient3292"
          x1="185"
          x2="346.2"
          y1="383.21"
          y2="383.21"
          gradientTransform="matrix(1 0 0 .1 0 305.57)"
          gradientUnits="userSpaceOnUse"
          // xlinkHref="#linearGradient3330"
        >
          <stop offset="0" stopOpacity="0" stop-color="white"></stop>
          <stop offset="0.25" stop-color="white"></stop>
          <stop offset="0.75" stopOpacity="0.839" stop-color="white"></stop>
          <stop offset="1" stopOpacity="0" stop-color="white"></stop>
        </linearGradient>
        <linearGradient
          id="linearGradient3296"
          x1="185"
          x2="346.2"
          y1="383.21"
          y2="383.21"
          gradientTransform="matrix(1 0 0 .1 -1.83 -222.12)"
          gradientUnits="userSpaceOnUse"
          // xlinkHref="#linearGradient3330"
        >
          <stop offset="0" stopOpacity="0" stop-color="white"></stop>
          <stop offset="0.25" stop-color="white"></stop>
          <stop offset="0.75" stopOpacity="0.839" stop-color="white"></stop>
          <stop offset="1" stopOpacity="0" stop-color="white"></stop>
        </linearGradient>
        <linearGradient
          id="linearGradient3300"
          x1="185"
          x2="346.2"
          y1="383.21"
          y2="383.21"
          gradientTransform="matrix(1 0 0 .1 -1.83 -382.32)"
          gradientUnits="userSpaceOnUse"
          // xlinkHref="#linearGradient3330"
        >
          <stop offset="0" stopOpacity="0" stop-color="white"></stop>
          <stop offset="0.25" stop-color="white"></stop>
          <stop offset="0.75" stopOpacity="0.839" stop-color="white"></stop>
          <stop offset="1" stopOpacity="0" stop-color="white"></stop>
        </linearGradient>
        <linearGradient
          id="linearGradient4180"
          x1="185"
          x2="346.2"
          y1="383.21"
          y2="383.21"
          gradientTransform="matrix(2.0533 0 0 .1 -746.69 -24.434)"
          gradientUnits="userSpaceOnUse"
          xlinkHref="#linearGradient3330"
        ></linearGradient>
        <linearGradient
          id="linearGradient4184"
          x1="185"
          x2="346.2"
          y1="383.21"
          y2="383.21"
          gradientTransform="matrix(2.9356 0 0 .1 -1029.7 -404.64)"
          gradientUnits="userSpaceOnUse"
          xlinkHref="#linearGradient3330"
        ></linearGradient>
        <linearGradient
          id="linearGradient4286"
          x1="185"
          x2="346.2"
          y1="383.21"
          y2="383.21"
          gradientTransform="matrix(.86848 0 0 .1 -314.06 -74.644)"
          gradientUnits="userSpaceOnUse"
          xlinkHref="#linearGradient3330"
        ></linearGradient>
        <linearGradient
          id="linearGradient4292"
          x1="185"
          x2="346.2"
          y1="383.21"
          y2="383.21"
          gradientTransform="matrix(1.1988 0 0 .1 -185.95 -190.34)"
          gradientUnits="userSpaceOnUse"
          xlinkHref="#linearGradient3330"
        ></linearGradient>
        <linearGradient
          id="linearGradient4298"
          x1="185"
          x2="346.2"
          y1="383.21"
          y2="383.21"
          gradientTransform="matrix(2.0933 0 0 .1 -876.24 -266.88)"
          gradientUnits="userSpaceOnUse"
          xlinkHref="#linearGradient3330"
        ></linearGradient>
        <linearGradient
          id="linearGradient4304"
          x1="185"
          x2="346.2"
          y1="383.21"
          y2="383.21"
          gradientTransform="matrix(.87007 0 0 .1 65.604 -524.43)"
          gradientUnits="userSpaceOnUse"
          xlinkHref="#linearGradient3330"
        ></linearGradient>
      </defs>
      <path
        fill="none"
        stroke="#000"
        strokeWidth="1"
        d="M14 13.888v472.226h138.39V152.29H344V13.888z"
        display="none"
      ></path>
      <path
        fill="none"
        stroke="#000"
        d="M183.8 343.9H344V183.65H183.8z"
        display="none"
      ></path>
      <path
        fill="url(#linearGradient3284)"
        d="M185 183.14H346.2V184.14H185z"
      ></path>
      <path
        fill="url(#linearGradient3292)"
        d="M185 343.4H346.2V344.4H185z"
      ></path>
      <path
        fill="url(#linearGradient3296)"
        d="M183.17 -184.3H344.37V-183.3H183.17z"
        transform="rotate(90)"
      ></path>
      <path
        fill="url(#linearGradient3300)"
        d="M183.17 -344.5H344.37V-343.5H183.17z"
        transform="rotate(90)"
      ></path>
      <path
        fill="none"
        stroke="#f0f"
        d="M-366.32 13.888v472.22h138.39v-333.82h191.61v-138.4z"
        display="none"
      ></path>
      <g transform="translate(380.32)">
        <path
          fill="url(#linearGradient4180)"
          d="M-366.82 13.388H-35.81999999999999V14.388H-366.82z"
        ></path>
        <path
          fill="url(#linearGradient4184)"
          d="M-486.61 -366.82H-13.379999999999995V-365.82H-486.61z"
          transform="rotate(-90)"
        ></path>
        <path
          fill="url(#linearGradient4286)"
          d="M-153.39 -36.822H-13.389999999999986V-35.822H-153.39z"
          transform="rotate(-90)"
        ></path>
        <path
          fill="url(#linearGradient4292)"
          d="M35.822 -152.52H229.062V-151.52H35.822z"
          transform="scale(-1)"
        ></path>
        <path
          fill="url(#linearGradient4298)"
          d="M-488.97 -229.06H-151.52000000000004V-228.06H-488.97z"
          transform="rotate(-90)"
        ></path>
        <path
          fill="url(#linearGradient4304)"
          d="M226.57 -486.61H366.83V-485.61H226.57z"
          transform="scale(-1)"
        ></path>
      </g>
      <g display="none" transform="translate(-181.46 -4.04)">
        <path
          fill="url(#linearGradient4180)"
          d="M-366.82 13.388H-35.81999999999999V14.388H-366.82z"
        ></path>
        <path
          fill="url(#linearGradient4184)"
          d="M-486.61 -366.82H-13.379999999999995V-365.82H-486.61z"
          transform="rotate(-90)"
        ></path>
        <path
          fill="url(#linearGradient4286)"
          d="M-153.39 -36.822H-13.389999999999986V-35.822H-153.39z"
          transform="rotate(-90)"
        ></path>
        <path
          fill="url(#linearGradient4292)"
          d="M35.822 -152.52H229.062V-151.52H35.822z"
          transform="scale(-1)"
        ></path>
        <path
          fill="url(#linearGradient4298)"
          d="M-488.97 -229.06H-151.52000000000004V-228.06H-488.97z"
          transform="rotate(-90)"
        ></path>
        <path
          fill="url(#linearGradient4304)"
          d="M226.57 -486.61H366.83V-485.61H226.57z"
          transform="scale(-1)"
        ></path>
      </g>
    </motion.svg>
  );
};

export default SvgLogoFgradient;
