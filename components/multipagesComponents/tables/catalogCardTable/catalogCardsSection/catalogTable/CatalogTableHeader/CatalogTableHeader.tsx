          ...
          index === 0 ? (
            ...
            >
              {Array.isArray(lab) &&
                lab.map((text, i) => (
                  <div key={i}>
                    <p className={`${tableTextS} text-grey`}>{text}</p>
                  </div>
                ))}
            </div>
          ) : (
                  ...
                  <div key={i} className="fc">
                    <p
                      className={`text-center ${
                        lab.length - 1 === i ? tableTextM : tableTextL
      ...
      <div className={headerStylesSwitcher(headerType)?.containerStyle}>
        {headerBottomData.map((lab, index) => {
            ...
            return (
              ...
              >
                {lab.map((text, index) => (
                  <div
                    key={index}
                    className="fc bg-greyShade1 py-2 border-x-[1px] border-dark"
                  >
                    <div className="fc w-[95%] h-full">
                      <p
                        className={`${tableTextS} text-center text-light break-all`}
                      >
                        {text}
                      </p>
                    </div>
          ...
          if (index !== 0 && Array.isArray(lab)) {
            return (
              <div
                className="fc flex-col border-x-[1px] border-dark"
                key={index}
              >
                {lab.map((text, index) => (
                  <div key={index} className="fc h-[50%] py-1">
                    <p className={`${tableTextS} text-grey`}> {text}</p>
                  </div>
                ))}
              </div>
